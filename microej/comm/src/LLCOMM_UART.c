...
int32_t LLCOMM_CUSTOM_CONNECTION_IMPL_sendData(LLCOMM_CUSTOM_CONNECTION *env, int32_t data)
{
    ...
    if (comm->enableTx == 1)
    {
        if(tcflush(comm->fd, TCOFLUSH) < 0)
        {
            // Look up errno to see error
            return -1;
        }
...
static void LLCOM_UART_IMPL_rxData_polling_func()
{
    ...
    while (uart_opened)
    {
        ...
        if ((fds[0].revents & POLLIN) && comm->enableRxInterrupt)
        {
            puts("***");
            LLCOMM_CUSTOM_CONNECTION_dataReceived(&(comm->header));
        }
        else if ((fds[0].revents & POLLOUT) && comm->enableTxInterrupt)
        {
